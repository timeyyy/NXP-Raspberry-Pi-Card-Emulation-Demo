...
#define PHHAL_HW_PN512_OPTION_RXTX_TIMER_START  0x0010U
	...
	*/
	typedef struct
	{
        ...
        void  * pOsalDataParams;								/**< Pointer to the parameter structure of the OSAL layer. */
		uint8_t * pTxBuffer;                                /**< Pointer to global transmit buffer used by the Exchange() function. */
		uint16_t wTxBufSize;                                /**< Size of the global transmit buffer. */
		uint16_t wTxBufLen;                                 /**< Number of valid bytes within the transmit buffer. */
		uint8_t * pRxBuffer;                                /**< Pointer to global receive buffer used by the Exchange() function. */
		...
		uint8_t bSensRes[2];		 						/**< Anticollision configuration: SENS_RES (ATQA) */
	    uint8_t bNfcId1[3];         						/**< Anticollision configuration: NFC ID1 (UID 3 bytes) */
	    uint8_t bSelRes;            						/**< Anticollision configuration: SEL_RES (SAK) */
	    uint8_t bPollRes[18];       						/**< Anticollision configuration: NFC ID2 (Felica) */
	    uint8_t bNfcId3;			 						/**< Anticollision configuration: NFC ID3 */
        uint16_t dwFdtPc;                                   /**< Current timing value backup for PC*/

        uint8_t bDontUseIrq;								/** Flag used to temporarily disable wait via interrupts */
	...
	*/
	phStatus_t phhalHw_Pn512_Init(
        ...
        void * pOsalDataParams,						/**< [In] Pointer to the OSAL layer parameter structure. */
		uint8_t * pTxBuffer,                      /**< [In] Pointer to global transmit buffer used by the Exchange() function. */
		uint16_t wTxBufSize,                      /**< [In] Size of the global transmit buffer. */
		uint8_t * pRxBuffer,                      /**< [In] Pointer to global receive buffer used by the Exchange() function. */
		uint16_t wRxBufSize                       /**< [In] Size of the global receive buffer. Specify the buffer +1 byte, because one byte is reserved for SPI communication. */
...
#define PHHAL_HW_CARDTYPE_ISO14443A_CE	    0x0009U /**< ISO/IEC 14443A Card Emulation Mode */
	/*@}*/
...
#define PHHAL_HW_MINFDT_848_US        83+45   /**< (n * 128 + 100) / 13.56; n = 8. */
	...
	*/
	...
	/**
	* \brief Listens for a new initiator and perform anticollison
	*
	* \return Status code
	* \retval #PH_ERR_SUCCESS Operation successful.
	*/
   phStatus_t phhalHw_Listen(
      void * pDataParams,     /**< [In]  Pointer to this layer's parameter structure. */
      uint8_t ** ppRxBuffer,  /**< [Out] Received Data */
	...
	/**
	* \brief Sets listen (anticollision) parameters
	*
	* \return Status code
	* \retval #PH_ERR_SUCCESS Operation successful.
	*/
	phStatus_t phhalHw_SetListenParameters(
		void * pDataParams,  /**< [In] Pointer to this layer's parameter structure. */
		uint8_t * pSensRes,  /**< [In] 2 byte SENS_RES */
		uint8_t * pNfcId1,   /**< [In] 3 byte NFC ID 1 (UID) */
		uint8_t  bSelRes,    /**< [In] SEL_RES (SAK) */
		uint8_t * pPollRes,  /**< [In] 18 byte NFC 2 */
		uint8_t  bNfcId3     /**< [In] NFC ID 3 */
		);
