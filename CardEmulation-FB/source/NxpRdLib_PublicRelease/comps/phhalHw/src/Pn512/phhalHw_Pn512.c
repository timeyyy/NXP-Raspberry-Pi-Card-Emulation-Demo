...
/* Default shadow for ISO/IEC 14443A Card Emulation Mode */
static const uint16_t PH_MEMLOC_CONST_ROM wPN512_DefaultShadow_ISO14443A_CE[][2] =
{
    {PHHAL_HW_CONFIG_PARITY,                PH_ON},
    {PHHAL_HW_CONFIG_TXCRC,                 PH_ON},
    {PHHAL_HW_CONFIG_RXCRC,                 PH_ON},
                              ...
                              )
{
	...
	pDataParams->bSelRes        		= 0;
	pDataParams->bNfcId3        		= 0;

	memset(&pDataParams->bSensRes[0], 0, 2);
	memset(&pDataParams->bNfcId1[0], 0, 3);
	memset(&pDataParams->bPollRes[0], 0, 18);

	pDataParams->bDontUseIrq			= 0;
                                  ...
                                  )
{
	...
	if(pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO14443A_CE)
	{
	    ...
	    if((pTxBuffer != NULL) && (pDataParams->wTxBufLen != 0))
	    {
	        /* Flush FiFo */
	        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_FlushFifo(pDataParams));
	    ...
	    {
	    	PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_ReadRegister(pDataParams, PHHAL_HW_PN512_REG_STATUS2, &bValue));
	        if((bValue & 0x07) == 0x01) // check if Modem State is Wait for StartSend
	        {
		...
		if (((status & PH_ERR_MASK) == PH_ERR_SUCCESS) && (NULL != ppRxBuffer))
		{
        ...
        else
        {
            ...
            if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
            {
                status = phhalHw_Pn512_ExchangeTransmit(pDataParams, PHHAL_HW_PN512_CMD_NOCMDCHANGE, pTxBuffer, wTxLength);
            }
        }
	}
    ...
    )
{
    ...
    case PHHAL_HW_CARDTYPE_I18092MPT:
        ...
        /* configure hardware for ISO/IEC 14443A CardEmu */
    case PHHAL_HW_CARDTYPE_ISO14443A_CE:

        /* configure Tx path for TypeA */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_TXMODE, PHHAL_HW_PN512_BIT_MIFARE_CE));
        /* configure Rx path for TypeA */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_RXMODE, PHHAL_HW_PN512_BIT_MIFARE_CE));
        /* configurate default TxModWidth */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_MODWIDTH, 0x26));
        /* configure the RxThreshold for TypeA  */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_RXTHRESHOLD, PHHAL_HW_PN512_RXTHRESHOLD_I14443A));
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_DEMOD, 0x61));

        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_MFTX, 0x6A));

        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, 0x25, 0x07));

        /* Use ISO/IEC 14443A CardEmu shadow */
        pShadowDefault = (uint16_t*)wPN512_DefaultShadow_ISO14443A_CE;
        wShadowCount = sizeof(wPN512_DefaultShadow_ISO14443A_CE) / (sizeof(uint16_t) * 2);
        break;
    ...
    if(pDataParams->bCardType == PHHAL_HW_CARDTYPE_I18092MPT) 
    {
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_RFCFG, 0x30));
    }
    else
    {
    ...
    if(pDataParams->bCardType == PHHAL_HW_CARDTYPE_ISO14443A_CE)
    {
    	...
    	PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_MODE, 0x3F));

        /* configure the conductance if no modulation is active */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_GSN, 0x6F)); // FF -> 6F
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_CWGSP, 0x20)); // 0x3F -> 20
    }
    else
    {
    	/* configure general settings */
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_WriteRegister(pDataParams, PHHAL_HW_PN512_REG_WATERLEVEL, PHHAL_HW_PN512_FIFOSIZE - 9));
                                   ...
                                   )
{
    ...
    case PHHAL_HW_CONFIG_SETMINFDT:
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_SetMinFDT(pDataParams, wValue));
        break;
    default:
...
//  /* Write the internal config buffer */
//  PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Pn512_Cmd_Config(pDataParams,
//                                              &pDataParams->bSensRes[0],
//                                              &pDataParams->bNfcId1[0],
//                                              pDataParams->bSelRes,
//                                              &pDataParams->bPollRes[0],
//                                              pDataParams->bNfcId3));

    /* Wait for a initiator */
   statusTmp = phhalHw_Pn512_Cmd_Autocoll(pDataParams, ppRxBuffer, pRxLength, bTimeoutUnit, wTimeout);
...
)
{
	...
	if ((NULL == pSensRes) || (NULL == pNfcId1)|| (NULL == pPollRes))
	{
		return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
	}

	/* save the new anticollison configuration parameters */
	memcpy(&pDataParams->bSensRes[0], pSensRes, 2);
	memcpy(&pDataParams->bNfcId1[0], pNfcId1, 3);
	memcpy(&pDataParams->bPollRes[0], pPollRes, 18);

	pDataParams->bSelRes = bSelRes;
	pDataParams->bNfcId3 = bNfcId3;
